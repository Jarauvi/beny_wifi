...
class BenyWifiConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_reconfigure(self, user_input = None):
        ...
        if user_input is not None:
            ...
            pin_is_valid = self._pin_is_valid(existing_data.get(SECTION_CONNECTION, {}).get(IP_ADDRESS), existing_data.get(SECTION_CONNECTION, {}).get(PORT), convert_pin_to_hex(user_input[SECTION_DEVICE][CONF_PIN]))
            if not pin_is_valid:
                self._errors["base"] = "wrong_pin"
